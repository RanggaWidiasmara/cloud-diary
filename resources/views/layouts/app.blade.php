...
{{-- 
    CEK ROLE USER: 
    Jika user memiliki role 'guru_bk' atau 'admin', tampilkan slot/content saja 
    (tanpa sidebar bawaan), karena sidebar admin sudah dibuat terpisah di admin.blade.php.
--}}
@if (request()->is('admin*'))
    <body class="bg-slate-50 text-slate-800 font-sans antialiased flex h-screen overflow-hidden">
        ...
        <div class="w-full h-full">
            @yield('content')
            {{-- Jika file menggunakan component <x-slot>, gunakan {{ $slot }} --}}
            {{ $slot ?? '' }} 
        </div>
    </body>
...
@else
    {{-- 
        TAMPILAN DEFAULT UNTUK SISWA ATAU GUEST (Belum Login)
        (Mempertahankan sidebar asli bawaan rekanmu) 
    --}}
    <body class="bg-slate-50 text-slate-800 font-sans antialiased flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        <!-- Sidebar Kiri (Responsive) untuk Siswa -->
